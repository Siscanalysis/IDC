...
default configuration and a 40,000-evaluation catalog budget. (The §7.3/§7.5
multi-objective **headline** cases promoted into the paper are re-run at the
matched **400,000**-evaluation budget and reported from
`mo_matched_budget_summary.csv`, not from this 40k catalog.) They are included
so the catalog claims in the paper are inspectable without re-running the full
sweep.
...
> tree; re-running the switches reproduces
...
| File | What it is |
|------|-----------|
| `mo_catalog_hv_igd.csv` | Multi-objective catalog: raw hypervolume (HV, higher better) per (problem, algorithm) at the 40,000-evaluation catalog budget — distinct from the §7.3/§7.5 paper headlines, which use the matched 400,000-evaluation budget and normalized HV (`mo_matched_budget_summary.csv`). IDC vs NSGA-II / NSGA-III / MOEA/D. (The `igd` column is a legacy extra; the paper ranks by HV and reports the geometric front-quality metrics, not IGD.) |
...
## Broader multi-objective catalog reads (raw HV, 40,000-eval catalog budget)

> These are the broader-catalog runs at the 40k catalog budget, **not** the
> paper's §7.3/§7.5 headline comparisons (those use the matched 400,000-eval
> budget and normalized HV, in `mo_matched_budget_summary.csv`).
...
`moeed13` is the validation case promoted into the manuscript (§7.3), where
it is re-run at the matched **400,000**-evaluation budget and reported as
normalized HV (`mo_matched_budget_summary.csv`); the 40k raw-HV read above is
the catalog version. The rest stay in the catalog.
...
#   suzuki_*) are run from the authors' MO drivers
#   and mirrored here.
```
